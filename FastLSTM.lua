...
-- Ref. A.: https://github.com/wojzaremba/lstm/blob/master/main.lua#L65
-- Expects 1D or 2D input.
-- The first input in sequence uses zero value for cell and hidden state
------------------------------------------------------------------------
local LSTM, parent = torch.class('nn.FastLSTM', 'nn.AbstractRecurrent')
...
function LSTM:buildModel()
   ...
   gates:add(
   gates:add(para):add(nn.CAddTable())
   ...
   -- hidden state
   local concat = nn.ConcatTable():add(nn.SelectTable(1)):add(nn.SelectTable(2))
   hidden:add(concat)
   ...
   seq:add(gates)
   local concat = nn.ConcatTable():add(hidden):add(nn.Identity())
   seq:add(concat)
   seq:add(nn.FlattenTable())


   -- cell state
   local concat = nn.ConcatTable():add(nn.SelectTable(3)):add(nn.SelectTable(2))
   hidden:add(concat)
